...
from app.core.auth import get_current_user, jwt_handler
...
async def notify_user(
    ...
    try:
        ...
        if sent:
            ...
            return {"status": "warning", "message": f"User {user_id} is not connected"}
    except Exception as e:
...
async def update_portfolio(
    ...
    try:
        portfolio_update = WebSocketMessage(
            ...
            data={"portfolio_id": portfolio_id, **data}
        )
...
async def websocket_test_page():
    """WebSocket test page for development only"""
    ...
    <body>
        ...
        <script>
            ...
            function authenticate() {
                ...
                if (ws && token) {
                    const message = { type: 'authenticate', data: { token: token } };
            ...
            function subscribeMarketData() {
                ...
                if (ws && symbol) {
                    const message = { type: 'market_data_subscribe', data: { symbols: [symbol] } };
                    ws.send(JSON.stringify(message));
                    addMessage(`Sent: ${JSON.stringify(message)}`);
                }
            }
            function unsubscribeMarketData() {
                const symbol = document.getElementById('symbol').value;
                if (ws && symbol) {
                    const message = { type: 'market_data_unsubscribe', data: { symbols: [symbol] } };
                    ws.send(JSON.stringify(message));
                    addMessage(`Sent: ${JSON.stringify(message)}`);
                }
            }
            function joinRoom() {
                ...
                if (ws && roomId) { addMessage(`Joining room: ${roomId}`); }
            }
            function leaveRoom() {
                const roomId = document.getElementById('room-id').value;
                if (ws && roomId) { addMessage(`Leaving room: ${roomId}`); }
            }
            function sendChatMessage() {
                const roomId = document.getElementById('room-id').value;
                const message = document.getElementById('chat-message').value;
                if (ws && roomId && message) {
                    const chatMessage = { type: 'chat_message', data: { room_id: roomId, message: message } };
